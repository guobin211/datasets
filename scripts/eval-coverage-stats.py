...
用法
----
  python3 scripts/eval-coverage-stats.py
  python3 scripts/eval-coverage-stats.py --csv evaluation/open-dataset/eval-formal.csv  # 同时校验 CSV 实际内容
"""
...
# 文件名含连字符，无法直接 import，按路径加载
_spec = importlib.util.spec_from_file_location(
    'build_formal_eval_csv', ROOT / 'scripts' / 'build-formal-eval-csv.py')
...
def funnel(norm_dir: Path, per_dataset: int, longbench_cap: int, seed: str):
    ...
    for r in dedup:
        stat[r['source']]['drop_train'] = stat[r['source']]['pool'] - stat[r['source']]['usable']
...
def main():
    ...
    ap.add_argument('--longbench-cap', type=int, default=B.SPECIAL_CAP['longbench-data'])
    ap.add_argument('--seed', default='formal-eval-2026')
    ap.add_argument('--out', help='markdown 输出路径（默认只打印）')
    args = ap.parse_args()

    raw, stat, final = funnel(Path(args.norm_dir), args.per_dataset,
                              args.longbench_cap, args.seed)

    lines = ['# 正式评测集覆盖度统计', '']
    lines.append(f'- 归一化总记录：**{len(raw):,}**')
    lines.append(f'- 本次采样入选：**{len(final):,}**')
    lines.append(f'- 配额：每数据集 {args.per_dataset} 条，longbench {args.longbench_cap} 条，'
    ...
    lines.append('')
    lines.append('> 「可用池」= 语种/有效性/去重/**train 划分**全部过滤后的候选量；'
                 '「入选」= 配额采样后条数。')
    lines.append('')
    rows = []
    ...
    rows.append(['**合计**', f"**{tot['raw']:,}**", f"**{tot['drop_code']:,}**",
                 f"**{tot['drop_lang']:,}**", f"**{tot['drop_invalid']:,}**",
                 f"**{tot['drop_dup']:,}**", f"**{tot['drop_train']:,}**",
                 f"**{tot['usable']:,}**", '—', f"**{tot['picked']:,}**",
                 f"**{tot['picked'] / tot['raw'] * 100:.1f}%**"])
    lines.append(md_table(
        ['数据集', '原始', 'code类', '非中英', '无效', '重复', 'train划分',
         '可用池', '配额', '入选', '覆盖率'],
        rows))
    lines.append('')
    ...
    if csv_path.exists():
        ...
        lines.append('质量抽查：')
        lines.append('')
        lines.append(md_table(['检查项', '结果'],
                              [[k, v] for k, v in d['quality'].items()]))
        lines.append('')
        lines.append('分数据集条数：')
        lines.append('')
        lines.append(md_table(['数据集', '条数'],
