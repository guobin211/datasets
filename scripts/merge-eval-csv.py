...
用法
----
  python3 scripts/merge-eval-csv.py a.csv b.csv --out merged.csv
  python3 scripts/merge-eval-csv.py a.csv b.csv --out merged.csv --no-dedupe
"""
...
def main():
    ...
    for p in args.inputs:
        ...
        with path.open(encoding='utf-8-sig', newline='') as fh:
            ...
            if header is None:
                ...
                sys.exit(f'列不一致：{path.name} 与首个文件不同\n'
                         f'  首个: {header}\n  当前: {cols}')
            n = 0
            for r in rd:
                rows.append(r)
    ...
    if not args.no_dedupe:
        ...
        for r in rows:
            ...
            kept.append(r)
        rows = kept
    print(f'  合并 {before:,} 行' + ('' if args.no_dedupe
                                     else f' → 去重后 {len(rows):,} 行'))
