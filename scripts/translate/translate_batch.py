...
----
  # 先看会发什么（不花钱）
  python3 scripts/translate/translate_batch.py --dry-run --limit 3
  # 跑小样
  TRANSLATE_API_KEY=sk-xxx python3 scripts/translate/translate_batch.py \
...
SCRIPT_RANGES = {
    'bn': (0x0980, 0x09FF), 'ar': (0x0600, 0x06FF), 'fa': (0x0600, 0x06FF),
    'am': (0x1200, 0x137F), 'gu': (0x0A80, 0x0AFF), 'kk': (0x0400, 0x04FF),
    'ru': (0x0400, 0x04FF), 'te': (0x0C00, 0x0C7F), 'th': (0x0E00, 0x0E7F),
    'ja': (0x3040, 0x30FF), 'ko': (0xAC00, 0xD7AF), 'zh': (0x4E00, 0x9FFF),
}

NUM_RE = re.compile(r'\d+(?:\.\d+)?')
LATEX_RE = re.compile(r'\$[^$]{1,200}\$|\\boxed\{[^}]{0,200}\}|\\[a-zA-Z]+\{[^}]{0,200}\}')
...
def verify(rec: dict, out: dict) -> list[str]:
    ...
    for k in ('question', 'solution'):
        if k in out and isinstance(out[k], str):
            if len(LATEX_RE.findall(src.get(k, ''))) != len(LATEX_RE.findall(out[k])):
...
def call_api(base: str, key: str, model: str, rec: dict, timeout: int,
...
def translate_one(rec: dict, base: str, key: str, model: str, timeout: int,
                  temperature: float, retries: int) -> tuple[dict, list[str]]:
    last = ''
    for attempt in range(retries):
        try:
            ...
            for k in keys:
                v = raw.get(k)
                empty = (not isinstance(v, list) or not v) if k == 'options' \
...
def main():
    ...
    ap.add_argument('--api-base', default=os.getenv('TRANSLATE_API_BASE',
    ...
    if args.dry_run:
        for r in todo[:3]:
            ...
            src, keys = build_payload(r)
            print('=' * 70)
            print(f"[{r['id']}] {r['source']} lang={r['lang']} 字段={keys}")
            print('--- USER PROMPT ---')
            print(msgs[1]['content'])
        print('=' * 70)
        total = sum(len(json.dumps(build_payload(r)[0], ensure_ascii=False)) for r in todo)
        print(f'dry-run：共 {len(todo):,} 条待译，payload 字符量 {total:,}')
        return

    if not args.api_key:
        print('错误：缺少 API key。设置环境变量 TRANSLATE_API_KEY 或用 --api-key 传入。',
              file=sys.stderr)
        sys.exit(1)

    ok = fail = 0
    t0 = time.time()
    with (done_path.open('a', encoding='utf-8') as fdone,
          failed_path.open('a', encoding='utf-8') as ffail,
          ThreadPoolExecutor(max_workers=args.concurrency) as ex):
        futs = {ex.submit(translate_one, r, args.api_base, args.api_key, args.model,
                          args.timeout, args.temperature, args.retries): r for r in todo}
        for i, fut in enumerate(as_completed(futs), 1):
            r = futs[fut]
            merged, problems = fut.result()
            if problems:
                fail += 1
                ffail.write(json.dumps({**r, '_error': problems}, ensure_ascii=False) + '\n')
            else:
                ok += 1
