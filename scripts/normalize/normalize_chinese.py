...
import sys, os, json, re, csv, ast, glob
...
ROOT = '/Users/guobin/tencent/datasets'
os.chdir(ROOT)
...
def record_sample(ds, rec):
    ...
    if len(samples[ds]) < 3:
        samples[ds].append({
            ...
            'lang': rec['lang'],
            'split': rec['split'],
            'metadata': {k: rec['metadata'].get(k) for k in list(rec['metadata'])[:6]},
        })
...
def norm_ceval():
    ...
    w = JsonlWriter(ds)
    orig = 0
    subj_dirs = [d for d in sorted((OPEN / 'ceval-data').iterdir())
            ...
            for p in sorted(subj.glob(f'{split}-*.parquet')):
                ...
                for row in t.to_pylist():
                    ...
                    try:
                        ...
                        ans_text = ''
                    meta = {'subject': subj.name, 'answer_letter': ans_letter,
                            'explanation': row.get('explanation', '')}
                    rec = make_record(
                        ds, 'chinese', 'mcq', q, ans_text,
                        options=options, solution=row.get('explanation', ''),
                        lang='zh', split=split, source_file=rel,
                        orig_id=str(row.get('id', '')), metadata=meta)
                    w.write(rec)
                    bump_empty(ds, rec)
            ...
            with open(p, encoding='utf-8') as f:
                ...
                for row in reader:
                    ...
                    options = [row.get('A', ''), row.get('B', ''),
                               row.get('C', ''), row.get('D', '')]
                    ans_letter = (row.get('Answer') or '').strip()
                    ans_text = row.get(ans_letter, '') if ans_letter in ('A', 'B', 'C', 'D') else ''
                    meta = {'subject': subject, 'answer_letter': ans_letter}
                    rec = make_record(
                        ds, 'chinese', 'mcq', q, ans_text,
                        options=options, lang='zh', split=split,
                        source_file=rel, orig_id=str(row.get('', '')), metadata=meta)
                    w.write(rec)
                    bump_empty(ds, rec)
                    record_sample(ds, rec)
    res = w.close()
    ...
    for p in sorted(d.glob('*.jsonl')):
        ...
        with open(p, encoding='utf-8') as f:
            for line in f:
                ...
                meta = {'context': r.get('context', ''), 'subtask': stem,
                        'length': r.get('length', ''), 'dataset': r.get('dataset', ''),
                        'language': r.get('language', ''), '_id': r.get('_id', ''),
                ...
                rec = make_record(
                    ds, 'chinese', 'long_context', q, ans_text,
                    lang=lang, split='test', source_file=rel,
                    orig_id=str(r.get('_id', '')), metadata=meta)
                w.write(rec)
                bump_empty(ds, rec)
                record_sample(ds, rec)
    res = w.close()
...
def norm_gaokao():
    ...
    w = JsonlWriter(ds)
    orig = 0
    groups = [('Data/Objective_Questions', 'objective'),
    ...
    for sub, kind in groups:
        for p in sorted((OPEN / 'GAOKAO-Bench' / sub).glob('*.json')):
            ...
            for e in data:
                ...
                meta = {'year': e.get('year', ''), 'category': e.get('category', ''),
                        'score': e.get('score', ''), 'index': e.get('index', ''),
                        'keywords': keywords, 'subject_file': fname, 'kind': kind}
                opts = None
                parsed_optmap = None
                if is_mcq_file:
                    ...
                    if parsed:
                        stem, optmap = parsed
                        opt_list = [optmap.get(L, '') for L in ['A', 'B', 'C', 'D', 'E']
                ...
                if opts:
                    ...
                    for L in ans_letters:
                        L = str(L).strip()
                        if parsed_optmap and L in parsed_optmap and parsed_optmap[L].strip():
                            ans_texts.append(parsed_optmap[L])
                    ans_text = ' '.join(ans_texts) if ans_texts else ' '.join(map(str, ans_letters))
                    meta['answer_letter'] = ' '.join(map(str, ans_letters))
                    rec = make_record(ds, 'chinese', 'mcq', q_use, ans_text,
                                      options=opts, solution=analysis, lang=lang,
                                      split='test', source_file=rel, metadata=meta)
                else:
                    # open_qa
                    ans_text = ' '.join(map(str, ans_letters)) if ans_letters else ''
                    rec = make_record(ds, 'chinese', 'open_qa', q, ans_text,
                                      solution=analysis, lang=lang, split='test',
                                      source_file=rel, metadata=meta)
                w.write(rec)
                bump_empty(ds, rec)
                record_sample(ds, rec)
    res = w.close()
    ...
    with open(p, encoding='utf-8') as f:
        for line in f:
            ...
            if not q:
                ...
                continue
            ref = r.get('reference', '')
            meta = {'question_id': r.get('question_id', ''),
                    'category': r.get('category', ''),
                    'subcategory': r.get('subcategory', ''),
                    'evidences': r.get('evidences', [])}
            rec = make_record(ds, 'chinese', 'instruction_following', q, ref,
                              lang='zh', split='test', source_file=rel,
                              orig_id=str(r.get('question_id', '')), metadata=meta)
            w.write(rec)
            bump_empty(ds, rec)
            record_sample(ds, rec)
    res = w.close()
...
def norm_safety():
    ...
    for lang, tag in (('zh', 'zh'), ('en', 'en')):
        ...
        if not test_p.exists() or not ans_p.exists():
            ...
            continue
        tests = json.load(open(test_p, encoding='utf-8'))
        answers = json.load(open(ans_p, encoding='utf-8'))
        ...
        for item in tests:
            ...
            if idx is not None and 0 <= idx < len(opts):
                ans_text = opts[idx]
            meta = {'category': item.get('category', ans_info.get('category', '')),
                    'answer_idx': idx}
            rec = make_record(ds, 'chinese', 'mcq', q, ans_text,
                              options=opts, lang=lang, split='test',
                              source_file=rel_t, orig_id=iid, metadata=meta)
            w.write(rec)
            bump_empty(ds, rec)
            record_sample(ds, rec)
    res = w.close()
    ...
    with open(p, encoding='utf-8') as f:
        for line in f:
            ...
            if not q:
                ...
                continue
            meta = {k: v for k, v in r.items() if k not in ('问题', '答案', 'question', 'answer')}
            rec = make_record(ds, 'chinese', 'open_qa', q, ans,
                              lang='zh', split='test', source_file=rel, metadata=meta)
            w.write(rec)
            bump_empty(ds, rec)
            record_sample(ds, rec)
    res = w.close()
...
def parse_hallu_mc(q):
    ...
    m = re.match(r'^Question\s*[:：]\s*(.*)$', q, re.S)
...
def norm_hallu():
    ...
    for e in load_json_array(p_open):
        ...
        if not q:
            ...
            continue
        bests = [e.get(f'Best Answer{i}', '') for i in range(1, 6) if e.get(f'Best Answer{i}')]
        ans_text = '\n'.join(bests)
        meta = {'source': e.get('Source', ''),
                'task': 'open'}
        rec = make_record(ds, 'chinese', 'open_qa', q, ans_text,
                          lang='zh', split='test', source_file=rel_open, metadata=meta)
        w.write(rec)
        bump_empty(ds, rec)
        record_sample(ds, rec)
    # mc
    ...
    for e in load_json_array(p_mc):
        ...
        if parsed:
            ...
            rec = make_record(ds, 'chinese', 'mcq', stem, ans_text,
                              options=opts, lang='zh', split='test',
                              source_file=rel_mc, orig_id=str(e.get('question_id', '')),
                              metadata={'answer_letter': ans_letter, 'task': 'mc'})
        else:
            rec = make_record(ds, 'chinese', 'open_qa', q, ans_raw,
                              lang='zh', split='test', source_file=rel_mc,
...
if __name__ == '__main__':
    for fn in (norm_ceval, norm_cmmlu, norm_longbench, norm_gaokao,
               norm_alignbench, norm_safety, norm_simpleqa, norm_hallu):
        try:
            r = fn()
            print(f"[OK] {r['dataset']}: rows={r['rows']} orig={r['orig_rows']} "
    ...
    with open(ROOT + '/scripts/normalize/stats_chinese.json', 'w', encoding='utf-8') as f:
