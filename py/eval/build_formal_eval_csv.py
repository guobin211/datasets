...
----
  ...
  python3 py/eval/build_formal_eval_csv.py --per-dataset 1000 --longbench-cap 300
  python3 py/eval/build_formal_eval_csv.py --no-cap            # 不做配额，全量导出
"""
...
# GSM 系列最终答案标记
GSM_FINAL = re.compile(r"####\s*(.+?)\s*$", re.DOTALL)
...
def load_normalized(norm_dir: Path) -> list[dict]:
    ...
    norm_dir 也可以直接指向单个 .jsonl 文件（配合 py/normalize/split_by_lang.py 的
    split/zh-en.jsonl 使用，避免把非中英文和未标注数据一起读进来）。
    """
    rows = []
    paths = [norm_dir] if norm_dir.is_file() else sorted(norm_dir.glob("*.jsonl"))
    for path in paths:
        name = path.stem
...
def is_real_letter_answer(answer: str, options: list) -> bool:
    ...
    return sum(len(o) for o in opts) / len(opts) > 3
...
def stratified_sample(
    pool: list[dict], cap: int, seed: str, split_policy: str
) -> list[dict]:
    """
    ...
    for b in buckets.values():
        b.sort(
            key=lambda r: (
                split_rank(r),
                stable_key(seed, f"{r['source']}::{r['_id']}"),
            )
        )
...
def main():
    ap = argparse.ArgumentParser(
        description="构建正式评测集 CSV（中英文优先，过滤无效数据）"
    )
    ap.add_argument("--out", default=str(DEFAULT_OUT), help="输出 CSV 路径")
    ap.add_argument("--norm-dir", default=str(NORM_DIR), help="归一化数据目录")
    ap.add_argument(
        "--per-dataset", type=int, default=DEFAULT_CAP, help="每数据集配额上限"
    )
    ap.add_argument(
        "--longbench-cap",
        type=int,
        default=SPECIAL_CAP["longbench-data"],
    ...
    # ---- 分类过滤 ----
    by_lang = [
        r for r in by_lang if (r.get("category") or "") not in EXCLUDE_CATEGORIES
    ]
    ...
    # ---- 稳定排序：source → 层内原顺序 ----
    final.sort(
        key=lambda r: (r["source"], stable_key(args.seed, f"{r['source']}::{r['_id']}"))
    )
