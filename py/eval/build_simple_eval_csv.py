...
用法：
  python3 py/eval/build_simple_eval_csv.py [--out 路径] [--max-steps 3] [--supplement 250]
...
def main() -> None:
    ...
    ap.add_argument(
        "--max-steps", type=int, default=3, help="英文补充题的推理步数上限（默认 3）"
    )
    ap.add_argument(
        "--supplement", type=int, default=250, help="英文补充题条数（默认 250）"
    )
    ...
    if len(en_rows) != len(zh_rows):
        print(
            f"警告：en/low {len(en_rows)} 条 != zh/low {len(zh_rows)} 条",
            file=sys.stderr,
        )
