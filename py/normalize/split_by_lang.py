...
用法
----
  python3 py/normalize/split_by_lang.py
  python3 py/normalize/split_by_lang.py --per-lang          # 非中英按语言再细分文件
  python3 py/normalize/split_by_lang.py --empty-lang other  # lang 为空并入非中英
"""
...
DIGIT_RANGES = {
    "阿拉伯-印度数字": (0x0660, 0x0669),
    "波斯数字": (0x06F0, 0x06F9),
    "孟加拉数字": (0x09E6, 0x09EF),
    "泰卢固数字": (0x0C66, 0x0C6F),
    "泰文数字": (0x0E50, 0x0E59),
    "梵文数字": (0x0966, 0x096F),
    "古吉拉特数字": (0x0AE6, 0x0AEF),
    "缅甸数字": (0x1040, 0x1049),
}
...
def main():
                ...
                for line in fh:
                    ...
                    if bucket == "unlabeled" and not args.keep_unlabeled:
                        ...
                        if key not in handles:
                            # 多输出句柄，统一在下方 finally 中关闭
                            handles[key] = open(  # noqa: SIM115
                                bucket_dirs[bdir] / f"{ds}.jsonl",
                                "w",
                                encoding="utf-8",
                            )
                    ...
                    if args.per_lang and bucket == "other":
                        if lang not in per_lang_handles:
                            # 多输出句柄，统一在 finally 中关闭
                            per_lang_handles[lang] = open(  # noqa: SIM115
    ...
    L += md_table(
        ["类别", "目录", "数据集数", "条数", "占比", "字符量", "平均字符/条"], rows
    )
    L.append("")
    ...
    L.append("")
    L.append(
        "这一类是评测集主体。现有 `evaluation/open-dataset/eval-formal.csv`"
        "（27,916 条）就是它经筛选后的产物——已验证："
        "以 `--norm-dir open-datasets/split/zh-en` 重跑，输出完全一致。"
    )
    L.append("")
    ds_rows = []
    for ds, c in sorted(per_ds["zh-en"].items(), key=lambda x: -sum(x[1].values())):
        n = sum(c.values())
        ds_rows.append(
            [
                ds,
    ...
    L.append("")
    L.append(
        "这一类是翻译对象。字符量为「题干+选项+答案+解析+context」合计，"
        "可用于估算翻译成本。"
    )
    L.append("")
    lang_rows = []
    ...
    L += md_table(
        ["语言码", "语言", "条数", "占比", "字符量", "平均字符/条"], lang_rows
    )
    L.append("")
    ...
    ods_rows.append(
        ["**合计**", f"**{other_n:,}**", f"**{sum(ds_chars.values()):,}**", "—", "—"]
    )
    ...
    if stat["unlabeled"]["records"]:
        ...
        if not args.keep_unlabeled:
            L.append(
                "> 默认**不落盘**，仅在此统计（用 `--keep-unlabeled` 可输出，"
                "约 1.7GB）。"
            )
            L.append("")
        L.append(
            "归一化时漏标 `lang` 的记录。经核查全部是 swe-bench / livecodebench 的"
            "英文 issue 描述 + git patch，属 `category=code`："
        )
        L.append("")
        u_rows = [
            [ds, f"{sum(c.values()):,}", "、".join(f"{k}" for k in c)]
            for ds, c in sorted(
                per_ds["unlabeled"].items(), key=lambda x: -sum(x[1].values())
            )
        ]
        ...
        L.append("")
        L.append(
            f"字符量 **{stat['unlabeled']['chars']:,}**，是真实外语数据的 "
    ...
    L.append("")
    if non_ascii_digit:
        other_digits = {l: n for (b, l), n in non_ascii_digit.items() if b == "other"}
        other_digits_total = sum(other_digits.values())
        L.append("### 5.1 非 ASCII 数字")
        L.append("")
        L.append(
            f"翻译队列中有 **{other_digits_total:,} 条**答案使用本土数字系统"
        ...
        L += md_table(
            ...
            [
                ...
                for l, n in sorted(other_digits.items(), key=lambda x: -x[1])
            ],
        )
        L.append("")
        zh_en_digits = {l: n for (b, l), n in non_ascii_digit.items() if b == "zh-en"}
        if zh_en_digits:
            detail = "、".join(
                f"{LANG_NAMES.get(l, l)} {n} 条"
                for l, n in sorted(zh_en_digits.items())
            )
            L.append(
                f"另外中英文类里也有 **{sum(zh_en_digits.values()):,} 条**"
    ...
    L.append("")
    L.append(
        "- **数学题的公式与数字不得改写**：`$...$`、`\\(...\\)`、`\\frac{}{}` 等"
        " LaTeX 片段必须原样保留。"
    )
    L.append(
        "- **答案字段按需翻译**：纯数字/表达式答案不应翻译；文本型答案翻译后"
        "需与选项索引对齐。"
    )
    L.append("- **选择题选项数量必须一致**：翻译后 `options` 长度应与原文相同。")
    L.append(
        "- **翻译完的校验**：走 `build-formal-eval-csv.py` 同一套有效性过滤，"
        "重点看「答案与选项对不上」这一类告警。"
    )
    L.append("")
    ...
    L.append("python3 py/eval/build_formal_eval_csv.py \\")
    ...
    L.append("python3 py/translate/translate_batch.py \\")
    ...
    L.append("")
    L.append("# 3) 合并两类")
    L.append("python3 py/eval/merge_eval_csv.py eval-zh-en.csv eval-translated.csv \\")
    L.append("    --out evaluation/open-dataset/eval-formal.csv")
    L.append("```")
    L.append("")
    L.append(
        "> 注意：`--norm-dir` 必须指向**目录**（内含 `<dataset>.jsonl`）。"
        "若指向单个合并文件，全部记录会被识别为同一数据集，"
        "2,000 条配额会把 22.8 万的池直接砍到 2,000 条。"
    )
    L.append("")
    L.append(
        "> 注：`build-formal-eval-csv.py` 的 `KEEP_LANGS` 目前只认 `zh`/`en`，"
    ...
    if non_ascii_digit:
        print(
            f"非 ASCII 数字答案  {sum(non_ascii_digit.values()):,} 条 "
