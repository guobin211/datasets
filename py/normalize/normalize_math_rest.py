...
def do_gsm_plus():
    ...
    orig = 0
    for name, split in [
        ("test-00000-of-00001.jsonl", "test"),
...
def parse_choices(field):
    ...
    out = []
    for it in items:
        m = re.match(r"^([A-Z])[.\)]\s*(.*)$", str(it).strip(), re.DOTALL)
        if m:
...
if __name__ == "__main__":
    for fn in [
        ...
        s = fn()
        print(
            f"[{s['dataset']}] rows={s['rows']} orig={s.get('orig_rows')} "
            f"skip={s['skipped']} size={s['size_mb']}MB"
        )
    stats_path = str(Path(__file__).with_name("stats_math_rest.json"))
